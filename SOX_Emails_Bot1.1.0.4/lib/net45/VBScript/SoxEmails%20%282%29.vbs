...
objWSTF.Range("A1").AutoFilter 7, Replace(WScript.Arguments(10),"_", " ")
LastRowTF = objWSTF.Range("A1048576").end(-4162).row
VisibleRow = objWSTF.AutoFilter.Range.Offset(1).SpecialCells(12).Row

objWSTF.Range("A" & VisibleRow & ":AD"& LastRowTF).Copy
objWorksheet.Range("A1").PasteSpecial
objWorkbook.SaveAs "MRDD SOX Control Review"
objWorkbook.Close
Set range = objWSTF.Range("A" & VisibleRow & ":AD"& LastRowTF)
...
For i = LBound(newarray) to UBound(newarray)
	...
	'FileName = Left(newarray(i),4) & " SOX Control Review"
	Set objWorksheet = objWorkbook.Worksheets(1)
	objWSTF.Range("A1:AD"& LastRowTF).Copy
        objWorksheet.Range("A1").PasteSpecial
	Value = Left(newarray(i),4) & " SOX Control Review"
	objWorkbook.SaveAs WScript.Arguments(1) & "\" & Value
	objWorkbook.Close
	End If
	End If
Next
...
objWBF1.SaveAs WScript.Arguments(2)
objWBF1.Close
	...
	Set NFolder = fso.GetFolder(FolderPath)
		...
		For Each FileItem in colFiles
		    If(Instr(FileItem.Name,"1013")>0)Then
				...
				objWBF5.SaveAs WScript.Arguments(3)
                objWBF5.Close
	...
	Set NFolder = fso.GetFolder(FolderPath)
		...
		For Each FileItem in colFiles
			...
			If(Instr(FileItem.Name,"1041")>0)Then
				...
				objWBF8.SaveAs WScript.Arguments(4)
                objWBF8.Close
	...
	Set NFolder = fso.GetFolder(FolderPath)
		...
		For Each FileItem in colFiles
			...
			If(Instr(FileItem.Name,"1095")>0)Then
				...
				objWBF10.SaveAs WScript.Arguments(5)
                objWBF10.Close
...
'newarr = Array("Sherlock, Christopher", "Buttery, Heather", "Taylor, Kim", "Do, Thuy", "Watkins, Trish", "Watt, Ofelia", "Zapata, Elaine", "Timko, Michael", "Rogy, Sarah", "Kravetz, Emmy", "Pike, Sid")
'newarr1 = Array("Christopher Sherlock", "Heather Buttery", "Kim Taylor", "Thuy Do", "Trish Watkins", "Ofelia Watt", "Elaine Zapata", "Michael Timko", "Sarah Rogy", "Emmy Kravetz", " Sid Pike")
...
For i = LBound(newarr) to UBound(newarr)
	...
	Set objWorkbook = objExcel.Workbooks.Add()
	objWorkbook.SaveAs WScript.Arguments(1) & "\2001 " & Replace(newarr1(j),"_"," ") & " SOX Control Review"
	Set objWorksheet = objWorkbook.Worksheets(1)
